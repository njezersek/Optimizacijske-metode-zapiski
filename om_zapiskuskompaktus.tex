	...
	[{\textbf{Optimizacijski problem}}
		...
		[{Dopustni \\ $\Omega \neq \emptyset$}, align=center
			...
			[Omejen \\ \textit{kriterijska f. omejena}, align=center
				...
				[Neoptimalni \\ \textit{odprta meja} \\ $\max x$ \\ p.p. $0 < x < 1$, align=center]
			]
		]   
	] 
\end{forest}
...
$(\Omega, f, \text{min/max})$ je linearni porgram, če je $\Omega$ podana z linearnimi enakostmi in neenakostmi ($\leq$, $\geq$) in je $f$ linearna (\emph{ne dovolimo $<,>$}). 
...
\end{align*}
(\emph{$\gtrless$ je oznaka za neomejeno vrednost})
...
\begin{align*}
	\begin{array}{l r@{\;} l >{\scriptsize}l}
		...
		\text{p.p.} & \sum_{i=1}^m a_{ij} y_i 	&\geq c_j	& \forall j=1,\dots , n'\\
...
$f: K^{\text{konv, odp}} \to \mathbb{R}$ odvedljiva
...
\subsubsection{Karush–Kuhn–Tuckerjevi pogoji}

\textbf{Lagrangeeva funkcija}
\[L(x,\lambda) = f(x) + \sum_{j=1}^m \lambda_j g_j (x)\]

$x^* \in \Omega$ zadošča pogojem KKT, če $\exists \lambda_1, \dots, \lambda_m \geq 0$, da velja:
\begin{align*}
	L_{x_i}(x^*) &= 0 & \forall i = 1, \dots, n \\
	\lambda_j g_j(x^*) &= 0 & \forall j = 1, \dots, m \\
	g_j(x^*) &\leq 0 & \forall j = 1, \dots, m \\
\end{align*}
\emph{V splošnem KKT pogoji niso ne potrebni ne zadostni za globalni/lokalni ekstrem.}


\textbf{Potrebnost pogojev KKT optimalnost:} \\
Če ima $f$ v točki $x^* \in D$ lokalni min na $D$ in so vezi v $x^*$ regularne, točka $x^*$ zadošča pogojem KKT.

\textbf{Zadostni pogoji za regularnost vezi:} \\
Če velja vsaj en od pogojev so vezi v $x^*$ regularne.
\begin{itemize}
	\item $g_1, \dots, g_m$ afine
	\item $\Omega$ konveksna, $f, g_1, \dots, g_m$ konveksne in notranjost $ D^\circ \neq \emptyset$
	\item vse množice vektorjev $\{\nabla g_i(x^*) \ |\ g_i(x^*) = 0\}$ linearno neodvisne
\end{itemize}

\textbf{Zadostnost pogojev KKT za optimalnost:} \\
$\Omega$ odprta, konveksna množica, $f, g_1, \dots, g_m : \Omega \to \mathbb{R}$ konveksne in odvedljive \\
Če $x^*$ ustreza KKT pogojem, je globalni minimum.

Za konveksni problem z $D^\circ \neq \emptyset$ so KKT pogoji \\$\iff$ $x^*$ globalni minimum.
